 ...
 * <code>double</code>) to numeric values.
 * </p>
 * 
 * <p>
...
public final class AddConverter extends UnitConverter {
	 ...
	 */
	public AddConverter(final double offset) {
		if ((float) offset == 0.0)
	...
	public UnitConverter concatenate(final UnitConverter converter) {
		if (converter instanceof AddConverter) {
			final double offset = _offset + ((AddConverter) converter)._offset;
	...
	private static UnitConverter valueOf(final double offset) {
		final float asFloat = (float) offset;
		return asFloat == 0.0f ? UnitConverter.IDENTITY : new AddConverter(
				offset);
	}
