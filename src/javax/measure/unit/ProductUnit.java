 ...
 * factorization). For example: <code>METER.pow(2).divide(METER)</code> returns
 * <code>METER</code>.
 * </p>
 *
...
public final class ProductUnit<Q extends Quantity> extends DerivedUnit<Q> {
	 ...
	 */
	public ProductUnit(final Unit<?> productUnit) {
	 ...
	 */
	private ProductUnit(final Element [] elements) {
		_elements = elements;
	}
	...
	private static Unit<? extends Quantity> getInstance(
			final Element [] leftElems,
			final Element [] rightElems) {

		// Merges left elements with right elements.
		final Element [] result = new Element [leftElems.length
				+ rightElems.length];
		int resultIndex = 0;
		for (int i = 0; i < leftElems.length; i++) {
			final Unit unit = leftElems[i]._unit;
			final int p1 = leftElems[i]._pow;
			final int r1 = leftElems[i]._root;
			int p2 = 0;
			int r2 = 1;
			...
			final int pow = (p1 * r2) + (p2 * r1);
			final int root = r1 * r2;
			if (pow != 0) {
				final int gcd = gcd(Math.abs(pow), root);
		...
		for (int i = 0; i < rightElems.length; i++) {
			final Unit unit = rightElems[i]._unit;
		...
		if (resultIndex == 0) {
			...
			final Element [] elems = new Element [resultIndex];
	 ...
	 */
	static Unit<? extends Quantity> getProductInstance(final Unit<?> left,
			final Unit<?> right) {
		Element [] leftElems;
	 ...
	 */
	static Unit<? extends Quantity> getQuotientInstance(final Unit<?> left,
			final Unit<?> right) {
		Element [] leftElems;
		if (left instanceof ProductUnit) {
			leftElems = ((ProductUnit<?>) left)._elements;
		...
		Element [] rightElems;
		if (right instanceof ProductUnit) {
			final Element [] elems = ((ProductUnit<?>) right)._elements;
	 ...
	 *         if <code>n == 0</code>.
	 */
	static Unit<? extends Quantity> getRootInstance(final Unit<?> unit,
			final int n) {
		Element [] unitElems;
		if (unit instanceof ProductUnit) {
			final Element [] elems = ((ProductUnit<?>) unit)._elements;
			unitElems = new Element [elems.length];
			for (int i = 0; i < elems.length; i++) {
				final int gcd = gcd(Math.abs(elems[i]._pow), elems[i]._root * n);
	 ...
	 */
	static Unit<? extends Quantity> getPowInstance(final Unit<?> unit,
			final int n) {
		Element [] unitElems;
		if (unit instanceof ProductUnit) {
			final Element [] elems = ((ProductUnit<?>) unit)._elements;
			unitElems = new Element [elems.length];
			for (int i = 0; i < elems.length; i++) {
				final int gcd = gcd(Math.abs(elems[i]._pow * n), elems[i]._root);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Unit<? extends Quantity> getUnit(final int index) {
	 ...
	 *         <code>(index &lt; 0 || index &gt;= size())</code>.
	 */
	public int getUnitPow(final int index) {
	 ...
	 *         <code>(index &lt; 0 || index &gt;= size())</code>.
	 */
	public int getUnitRoot(final int index) {
	 ...
	 * @return <code>true</code> if <code>this</code> and <code>that</code> are
	 *         considered equals; <code>false</code>otherwise.
	 */
	@Override
	public boolean equals(final Object that) {
		...
		if (that instanceof ProductUnit) {
			...
			final Element [] elems = ((ProductUnit<?>) that)._elements;
	...
	private boolean hasOnlyStandardUnit() {
		for (int i = 0; i < _elements.length; i++) {
			final Unit<?> u = _elements[i]._unit;
			if (!u.isStandardUnit())
				return false;
		}
		return true;
	...
	private final static class Element implements Serializable {
		...
		private Element() {
			/* Not for public use, only serialization via Kryo. */
			_unit = null;
			_root = 0;
			_pow = 1;
		}
		 ...
		 *        the root exponent.
		 */
		private Element(final Unit<?> unit, final int pow, final int root) {
			_unit = unit;
			_pow = pow;
			_root = root;
		}
