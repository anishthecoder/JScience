 ...
 * Copyright (C) 2007 - JScience (http://jscience.org/) All rights reserved.
 * 
...
/**
 * <p>
 * This class represents a measurement vector of two or more dimensions. For
 * example:[code] VectorMeasure<Length> dimension = VectorMeasure.valueOf(12.0,
 * 30.0, 40.0, MILLIMETER); VectorMeasure<Velocity> v2d =
 * VectorMeasure.valueOf(-2.2, -3.0, KNOTS); VectorMeasure<ElectricCurrent> c2d
 * = VectorMeasure.valueOf(-7.3, 3.5, NANOAMPERE); [/code]
 * </p>
 * 
 * <p>
 * Subclasses may provide fixed dimensions specializations:[code] class
 * Velocity2D extends VectorMeasure<Velocity> { public Velocity2D(double x,
 * double y, Unit<Velocity> unit) { ... } } [/code]
 * </p>
 * 
 * <p>
 * Measurement vectors may use {@link CompoundUnit compound units}:[code]
 * VectorMeasure<Angle> latLong = VectorMeasure.valueOf(12.345, 22.23,
 * DEGREE_ANGLE); Unit<Angle> HOUR_MINUTE_SECOND_ANGLE =
 ...
 */
public abstract class VectorMeasure<Q extends Quantity> extends
		Measure<double [], Q> {
	...
	public static <Q extends Quantity> VectorMeasure<Q> valueOf(
			final double x, final double y, final Unit<Q> unit) {
	 ...
	 */
	public static <Q extends Quantity> VectorMeasure<Q> valueOf(
			final double x, final double y, final double z, final Unit<Q> unit) {
	...
	/**
	 * Returns the <code>String</code> representation of this measurement vector
	 * (for example <code>[2.3 m/s, 5.6 m/s]</code>).
	 * 
	...
	private static class TwoDimensional<Q extends Quantity> extends
			VectorMeasure<Q> {
		...
		private TwoDimensional(final double x, final double y,
				final Unit<Q> unit) {
			_x = x;
			_y = y;
			_unit = unit;
		...
		public double doubleValue(final Unit<Q> unit) {
			final double norm = Math.sqrt(_x * _x + _y * _y);
		...
		@Override
		public TwoDimensional<Q> to(final Unit<Q> unit) {
			if ((unit == _unit) || (unit.equals(_unit)))
				return this;
			final UnitConverter cvtr = _unit.getConverterTo(unit);
			return new TwoDimensional<Q>(cvtr.convert(_x), cvtr.convert(_y),
					unit);
		}
	...
	private static class ThreeDimensional<Q extends Quantity> extends
			VectorMeasure<Q> {
		...
		private ThreeDimensional(final double x, final double y,
				final double z, final Unit<Q> unit) {
			_x = x;
			_y = y;
			_z = z;
			_unit = unit;
		...
		@Override
		public double doubleValue(final Unit<Q> unit) {
			final double norm = Math.sqrt(_x * _x + _y * _y + _z * _z);
			if ((unit == _unit) || (unit.equals(_unit)))
				return norm;
			return _unit.getConverterTo(unit).convert(norm);
		}
		...
		@Override
		public ThreeDimensional<Q> to(final Unit<Q> unit) {
			if ((unit == _unit) || (unit.equals(_unit)))
				return this;
			final UnitConverter cvtr = _unit.getConverterTo(unit);
			return new ThreeDimensional<Q>(cvtr.convert(_x), cvtr.convert(_y),
					cvtr.convert(_z), unit);
		}
	...
	private static class MultiDimensional<Q extends Quantity> extends
			VectorMeasure<Q> {
		...
		private MultiDimensional(final double [] components, final Unit<Q> unit) {
		...
		@Override
		public MultiDimensional<Q> to(final Unit<Q> unit) {
			if ((unit == _unit) || (unit.equals(_unit)))
				return this;
			final UnitConverter cvtr = _unit.getConverterTo(unit);
			final double [] newValues = new double [_components.length];
